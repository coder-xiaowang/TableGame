...
const E = Object.fromEntries(["hero","connectionStatus","roomHeaderTools","setupPanel","roomPanel","hostModeButton","guestModeButton","hostSetup","guestSetup","hostNameInput","guestNameInput","playerCountSelect","createRoomButton","joinRoomButton","roomCodeInput","joinIntentField","roomCodeDisplay","hostTools","roomPlayerCountSelect","spectatorSettingButton","seatActionButton","spectatorPanel","spectatorCountBadge","spectatorList","startGameButton","restartGameButton","endGameButton","notice","westernTable","players","tableCenter","centerMoment","deckPile","deckCount","discardTop","presentationEffects","presentationTrail","presentationAnnouncement","presentationLabel","presentationText","controlDock","actionTitle","actionHint","actionButtons","timerText","timerBar","privateZone","selfSummary","myHand","toggleLogButton","logList"].map((id)=>[id,$(id)]));

let view = null, selectedCardId = null, selectedTargetId = null, spectatorUi = null, presentation = null;
...
const room = createAuthoritativeRoomClient({
  ...
  handlers:{ onView(next){ view=next; const mine=own()?.hand.some((card)=>card.id===selectedCardId); if(!mine){selectedCardId=null;selectedTargetId=null;} enterRoom();render();presentation?.sync(next.presentationEvents); }, onKicked(){spectatorUi?.handleSessionEnded("kicked");}, onRoomExpired(){spectatorUi?.handleSessionEnded("room_expired");} }
});
...
function seatFor(id){return [...E.players.querySelectorAll("[data-player-id]")].find((seat)=>seat.dataset.playerId===String(id))||null;}
function eventSource(event){if(event.kind==="private-draw"){if(event.source==="steal")return seatFor(event.targetId);if(event.source==="discard")return E.discardTop;return E.deckPile;}if(["draw","character-draw"].includes(event.kind))return E.deckPile;if(["draw-steal","character-steal","inherit-cards"].includes(event.kind))return seatFor(event.targetId);if(event.kind==="draw-discard")return E.discardTop;return seatFor(event.actorId);}
function eventTarget(event){if(["draw","private-draw","draw-steal","draw-discard","character-draw","character-steal","inherit-cards"].includes(event.kind))return seatFor(event.actorId);return seatFor(event.targetId)||E.tableCenter;}
function presentationLabel(kind){return {"match-start":"枪战开始","turn-start":"轮到行动",draw:"摸牌阶段","draw-steal":"角色能力","draw-discard":"角色能力","character-draw":"角色能力","character-steal":"角色反击","inherit-cards":"接收遗牌","sheriff-penalty":"警长惩罚","private-draw":"私人情报",attack:"枪声响起","response-window":"等待响应",defense:"成功防御",barrel:"木桶判定",judgment:"命运判定",duel:"发起决斗","duel-response":"决斗响应","duel-return":"决斗还击","group-attack":"全桌危机",heal:"恢复生命","group-heal":"酒馆疗伤",steal:"抢劫", "discard-target":"拆除", "private-card-transfer":"私人结果",equipment:"装备生效","general-store":"杂货店","store-choice":"公开选牌",damage:"受到伤害",dying:"濒死求援",rescue:"紧急救援",elimination:"身份揭晓","match-result":"枪战落幕",discard:"回合收尾","card-play":"卡牌生效"}[kind]||"西部动态";}
function playPresentationObject(event){
  const source=eventSource(event),target=eventTarget(event);
  const highlighted=[source,target].filter(Boolean);highlighted.forEach((element)=>element.classList.add("presentation-focus"));
  if(["attack","duel","group-attack","damage","dying","elimination"].includes(event.kind))E.westernTable.classList.add("danger-moment");
  let token=null;
  if(source&&target&&source!==target){const stage=E.westernTable.getBoundingClientRect(),from=source.getBoundingClientRect(),to=target.getBoundingClientRect();token=document.createElement("span");token.className="presentation-token";token.textContent=event.cardType?cardName(event.cardType):event.kind==="rescue"?"♥":"★";token.style.setProperty("--from-x",`${from.left+from.width/2-stage.left}px`);token.style.setProperty("--from-y",`${from.top+from.height/2-stage.top}px`);token.style.setProperty("--to-x",`${to.left+to.width/2-stage.left}px`);token.style.setProperty("--to-y",`${to.top+to.height/2-stage.top}px`);E.presentationEffects.append(token);}
  return()=>{highlighted.forEach((element)=>element.classList.remove("presentation-focus"));E.westernTable.classList.remove("danger-moment");token?.remove();};
}
...
async function kickPlayer(playerId){const target=player(playerId);if(!target||target.isHost||!confirm(`确定移出 ${target.name} 吗？`))return;try{await room.kick(playerId);}catch(error){alert(error.message);}}
...
function renderPlayers(){
  ...
  const leftCount=Math.floor(seats.length/2),rightCount=seats.length-leftCount;
  E.players.dataset.count=String(seats.length);
  E.players.innerHTML=seats.map((p,index)=>{const side=index<leftCount?"left":"right",sideIndex=side==="left"?index:index-leftCount,sideCount=side==="left"?leftCount:rightCount,y=sideCount===1?50:8+84*(sideIndex+.5)/sideCount;const responding=view.pending?.actorId===p.id&&["defense","duel","dying"].includes(view.phase),source=view.pending?.sourceId===p.id;return `<article data-player-id="${escapeHtml(p.id)}" data-side="${side}" style="--seat-y:${y}%" class="player-seat ${p.id===view.selfId?"self":""} ${p.id===view.currentPlayerId?"current":""} ${responding?"responding":""} ${source?"action-source":""} ${!p.alive?"dead":""} ${!p.connected?"offline":""}"><div class="seat-head"><div><b>${escapeHtml(p.name)}${p.id===view.selfId?" · 你":""}</b><small>${p.connected?"在线":"离线"} · ${p.handCount}张手牌${p.distance?` · 距离${p.distance}`:""}</small></div><div class="seat-tools"><span class="role">${escapeHtml(p.role?ROLE[p.role]:"身份隐藏")}</span>${view.permissions.canManage&&view.phase==="lobby"&&!p.isHost?`<button class="kick small" data-kick="${escapeHtml(p.id)}">移出</button>`:""}</div></div><div class="character">${escapeHtml(p.characterName||"等待角色")} <span class="life">${p.alive?"♥".repeat(Math.max(0,p.life)):"已出局"}${p.maxLife?` / ${p.maxLife}`:""}</span></div><div class="equipment">${p.equipment.map((card)=>`<span class="equip">${escapeHtml(cardName(card.type))}</span>`).join("")||"<small>无装备</small>"}</div></article>`;}).join("");
