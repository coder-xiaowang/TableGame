...
game5 使用共享的服务端演出事件流表现出牌、罚牌指向、跳过、反转、质疑、喊 UNO、抓 UNO、摸牌和胜负。事件由权威状态统一编号并随 SQLite 一起持久化；浏览器只负责播放，不参与规则结算，刷新或启用“减少动态效果”都不会改变牌局结果。

- 公共事件只携带已经公开的牌、动作发起者、目标、罚牌数量与结算结果。
- 起手牌和摸到的具体牌使用每位玩家独立的私人事件，只进入该玩家视图；旁观者和其他玩家不会收到。
- `+4` 的合法性在质疑结算前仍只保存在服务端，演出事件不会提前泄露。
- 首次加入和断线重连会从最新事件序号继续，不会重播整局历史。
- 同一次玩家操作合并为一个场景；队列积压时自动加速，严重积压时舍弃低价值旧场景，罚牌响应、质疑和胜负等关键事件仍会保留。
