...
test("game3 supports voluntary spectators and safe lobby seat changes", async (context) => {
  ...
  assert.equal(observer.view.capacity, 3);
...
test("game3 full and active rooms admit read-only spectators with the public dice", async (context) => {
  const running = await launch();
  const streams = [];
  ...
  assert.equal((await post(running.base, "/api/actions", auth(host, {
    actionId: "roll", action: { type: "roll" }
  }))).response.status, 200);
  const playerView = (await post(running.base, "/api/join", {
    roomCode: host.roomCode, clientId: guest.clientId, resumeToken: guest.resumeToken
  })).payload.view;
  ...
  assert.deepEqual(late.payload.view.dice, playerView.dice);
  assert.deepEqual(late.payload.view.held, playerView.held);
  assert.deepEqual(late.payload.view.players, playerView.players);
  const forged = await post(running.base, "/api/actions", auth(late.payload, {
    actionId: "forged", action: { type: "roll" }
  }));
...
test("game3 serializes concurrent competition for the last player seat", async (context) => {
  const running = await launch();
  context.after(async () => { if (running.server.listening) await close(running.server); });
  const host = await createRoom(running, 2);
  const observers = [];
...
test("game3 SQLite restart restores spectator identity and room setting", async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "tablegame-game3-spectator-"));
...
test("game3 page consumes shared spectator UI and renders spectator action guidance", () => {
