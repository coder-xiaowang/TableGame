...
服务端会为密文征集、封存提交、分发代号、提问、回答、猜词判定、陷阱触发、跳过与设备复位生成递增编号的公开演出事件。浏览器使用共享演出时间线播放电报轨迹、线路回传、译码标记和判定盖章；首次加入或断线重连只建立事件游标，不会重播历史。

演出事件不参与规则推进，也不会携带答案词、陷阱词或玩家提交内容。快速连续回答时播放器会缩短待播场景，严重积压时保留最新事件和高优先级判定，避免画面长期落后于服务端状态。迁移边界与验收记录见 `PRESENTATION_MIGRATION.md`。
