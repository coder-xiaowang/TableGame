...
test("SQLite重启后恢复牌局、截止时间和旁观者隐私视图", async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "tablegame-game18-"));
  const filename = path.join(directory, "game18.sqlite");
  const engine = await import("./server/game-engine.mjs");
  let running = null;
  let streams = [];
  context.after(async () => {
    if (running?.server.listening) await close(running, streams);
  ...
  const p3 = (await post(running.base, "/api/join", { roomCode: host.roomCode, clientId: "p3", name: "丙" })).payload;
  const watcher = (await post(running.base, "/api/join", { roomCode: host.roomCode, clientId: "watch", name: "旁观", intent: "spectate" })).payload;
  streams = [await events(running.base, host), await events(running.base, p2), await events(running.base, p3)];
  ...
  assert.deepEqual(after.players.find((player) => player.id === host.clientId).hand, before.players.find((player) => player.id === host.clientId).hand);
  ...
  assert.equal(watched.privateInsight, null);
});
