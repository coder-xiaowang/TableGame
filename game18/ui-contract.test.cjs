...
test("页面满足圆桌中央行动区、旁观与移动端紧凑列表契约", () => {
  ...
  assert.match(html, /id="privateZone" class="compact-hand-zone"/);
  assert.ok(html.indexOf('id="privateZone"') > dockAt && html.indexOf('id="privateZone"') < html.indexOf("</section>", html.indexOf('id="privateZone"')), "手牌必须整合在中央操作单元内");
  ...
  assert.match(styles, /\.compact-hand-zone\s*\{/);
  assert.match(styles, /@media \(max-width: 760px\)/);
  assert.match(styles, /\.player-seat \{ position: static;[\s\S]*?transform: none;/);
  assert.match(styles, /@media \(max-width: 520px\)/);
  assert.match(styles, /\.compact-hand-zone \.game-card:not\(\.compact\)[\s\S]*?width: 76px/);
  assert.match(styles, /\.players\[data-count="7"\][\s\S]*?grid-template-columns: repeat\(4,minmax\(120px,190px\)\)/);
  assert.match(styles, /width: clamp\(320px,34%,420px\)/);
