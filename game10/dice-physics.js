...
export async function createDicePhysics({ canvas } = {}) {
  ...
  const materials = FACE_VALUES.map((value) => new THREE.MeshStandardMaterial({ map:makeFaceTexture(value), roughness:.7, metalness:0 }));
  ...
  function play(simulation, { elapsedMs = 0 } = {}) {
  ...
  function dispose() {
    ...
    materials.forEach((material) => { material.map?.dispose(); material.dispose(); }); renderer.dispose();
  }
  return Object.freeze({ play, hide, dispose, renderFinal(simulation) { play(simulation, { elapsedMs:simulation.durationMs }); } });
}
