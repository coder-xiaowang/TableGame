...
game10 已接入共享服务端演出事件。掷骰、选择路线、扎营、爆掉、占领峰顶和胜负都由状态机在实际生效时生成公开事件；客户端只负责方向轨迹、路线聚焦和中央播报。原有三维骰子及登山者动画继续承担物件运动，不会重复生成另一套骰子。播放队列会在连续操作时追赶并优先保留爆掉、占峰和胜负。事件清单与实机验收见 `PRESENTATION_MIGRATION.md`。
