...
async function resume(running, session) {
  return post(running.base, "/api/join", {
    roomCode: session.roomCode,
...
test("SQLite restart restores the room, private hands, deadline and action deduplication", async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "tablegame-dirty-pig-"));
  ...
  const duplicate = await action(running, host, "start-once", { type: "end" });
