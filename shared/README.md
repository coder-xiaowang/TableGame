...
- `client/spectator-ui.js`：旁观加入、身份提示、换席、旁观席、房主设置和统一错误信息。
- `client/presentation-events.js`：服务器演出事件的去重、排序、播放队列、中央通告和来源—目标轨迹。
- `server/presentation-events.mjs`：为权威演出事件分配不可伪造的递增序号、裁剪历史并兼容旧快照。
...
- `styles/spectator.css`：可按游戏主题变量覆盖的通用旁观界面样式。

公共层不维护游戏 `state`，也不实现动作校验、阶段推进、隐私视图、胜负结算或游戏界面。

演出时间线首次收到房间快照时只建立事件游标，因此加入或断线重连不会重播历史动画；后续仅播放 `sequence` 更大的事件。每款游戏可以注入事件名称、来源和目标元素，以及卡牌强调等专属演出。动画只表现已经由服务器裁定的结果，不能代替规则结算或携带不应公开的秘密字段。

需要应对快速操作的游戏可以选择启用自适应调度参数：通过 `sceneKey` 将同一次权威操作的多条事件合并为一个场景，通过 `priorityFor` 标记关键程度，并配置 `catchUpThreshold`、`severeBacklogThreshold`、追赶时长与紧急优先级。正常情况下完整播放；积压时缩短时长；严重积压时保留关键场景和最新两个场景；紧急响应到达时清除尚未播放的低优先级旧场景。所有参数默认关闭，不会改变尚未选择接入的游戏。
...
```js
import { createSpectatorUi } from "/shared/client/index.js";
```

游戏向它传入共享权威客户端、当前视图读取函数和页面挂载元素。通用模块负责：

- 根据 `/api/config` 开关加入身份选择；
- 统一解释旁观相关错误码和自动旁观原因；
- 渲染当前 `player` / `spectator` 身份；
- 渲染旁观席和在线状态；
- 调用 `changeSeat()`、`setRoomSettings()` 和旁观者移出接口；
- 根据服务端提供的 `canChangeSeats` 决定是否显示换席按钮；
- 在移动端默认折叠旁观席。

通用模块不会决定真实权限，也不会裁剪游戏隐藏信息。座位权限仍由共享服务器最终校验，公共旁观视图仍必须由每个游戏自己的 `buildSpectatorView()` 生成。
