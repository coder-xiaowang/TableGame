...
function pushEvent(clientId, event, payload) {
  ...
  if (!client) {
    ...
    pendingSignals.set(clientId, queue.slice(-30));
    return false;
  }
...
function flushPending(clientId) {
  const queue = pendingSignals.get(clientId) || [];
  queue.forEach((item) => pushEvent(clientId, item.event, item.payload));
...
server.listen(port, () => {
  console.log(`Idiom board game: http://localhost:${port}`);
