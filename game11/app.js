...
import {
  createCodeDeck, lastEligiblePlayer, normalizeClue, otherTeam, outcomeForTeams,
...
function makeLobby(capacity, host) {
  return { phase:"lobby", capacity, players:[host], teams:{white:teamState(),black:teamState()}, round:0, turnTeam:"white", encryptorId:"", code:null, clues:[], guesses:{decode:null,intercept:null}, deadline:0, records:[], outcome:null, tiebreakGuesses:{white:null,black:null} };
}
...
function beginTransmission(team) {
  hostTimer.clear(); state.turnTeam=team; state.phase="clue"; state.clues=[];state.guesses={decode:null,intercept:null};state.code=drawCode(team);
...
function scoreHtml(view,team){const t=view.teams[team];return `<div class="score-team"><h3>${TEAM_NAME[team]}</h3><div class="score-tokens">截获 ${"●".repeat(t.interceptions)}${"○".repeat(2-t.interceptions)}　误传 ${"◆".repeat(t.miscommunications)}${"◇".repeat(2-t.miscommunications)}</div></div>`;}
