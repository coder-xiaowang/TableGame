...
async function launch(engine, filename) {
  const server = startServer({ gameRoot: __dirname, sharedRoot: path.resolve(__dirname, "../shared"), engine, protocolVersion: 3, defaultPort: 0, spectatorsEnabled: true, roomStore: createSqliteRoomStore({ filename }) });
...
test("SQLite重启后恢复地点、身份、轮次、截止时间并继续隔离旁观者", async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "tablegame-game21-"));
  const filename = path.join(directory, "game21.sqlite");
  ...
  const players = [host];
  for (let index = 2; index <= 3; index += 1) players.push((await post(running.base, "/api/join", { roomCode: host.roomCode, clientId: `p${index}`, name: `玩家${index}` })).payload);
  ...
  const before = await Promise.all(players.map(async (session) => (await resume(running, session)).payload.view));
  const operativeBefore = before.find((item) => item.privateLocation);
  await close(running, streams);
  streams = [];

  running = await launch(engine, filename);
  const after = (await resume(running, players.find((session) => session.clientId === operativeBefore.selfId))).payload.view;
  const watched = (await resume(running, watcher)).payload.view;
  assert.equal(after.phase, operativeBefore.phase);
  assert.equal(after.round, operativeBefore.round);
  assert.equal(after.deadline, operativeBefore.deadline);
  assert.equal(after.privateRole, operativeBefore.privateRole);
  assert.equal(after.privateLocation.id, operativeBefore.privateLocation.id);
  assert.ok(after.players.every((player) => !player.connected));
  assert.equal(watched.roomRole, "spectator");
  assert.equal(watched.privateLocation, null);
  assert.ok(watched.players.every((player) => player.role === null && player.locationRole === null));
});
