...
game9 已接入共享的服务器权威演出事件层。拒绝时显示筹码由行动者移向牌面；拿牌时数字牌移向获得者，随后再从牌堆翻开新牌；开局与胜负结算使用更高优先级提示。所有演出只解释已经完成的权威结算，不会阻塞操作或改变倒计时。

公共事件不会携带其他玩家剩余筹码或秘密移除牌，玩家与旁观者看到一致的公开过程。首次进入和断线重连不会重播历史；快速操作积压时会自动加速并压缩低价值场景。详细边界与实机验收项见 [PRESENTATION_MIGRATION.md](PRESENTATION_MIGRATION.md)。
