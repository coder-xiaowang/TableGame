...
async function createRoom(running, capacity = 3) {
  const result = await post(running.base, "/api/rooms", {
...
test("game9 supports voluntary spectators and safe lobby seat changes", async (context) => {
  ...
  assert.deepEqual(playerLeaves.payload.view.players.map(({ id }) => id), ["host"]);
  assert.equal(playerLeaves.payload.view.spectatorCount, 2);
...
test("game9 full rooms and games in progress admit only redacted spectators", async (context) => {
  const running = await launch();
  const streams = [];
  ...
  for (const id of ["p2", "p3"]) {
    const joined = await post(running.base, "/api/join", {
      roomCode: host.roomCode, clientId: id, name: id, intent: "play"
    });
  ...
  for (const player of players) streams.push(await openEvents(running.base, player));
  const started = await post(running.base, "/api/actions", auth(host, {
    actionId: "start-with-observer", action: { type: "start" }
  }));
  ...
  const forged = await post(running.base, "/api/actions", auth(late.payload, {
    actionId: "spectator-forged", action: { type: "take" }
  }));
  ...
  const kicked = await post(running.base, "/api/kick", {
    ...
    hostId: host.clientId,
    resumeToken: host.resumeToken,
    playerId: fullObserver.clientId
  });
  assert.equal(kicked.response.status, 200);
});

test("game9 resolves concurrent competition for the last player seat", async (context) => {
  const running = await launch();
  context.after(async () => { if (running.server.listening) await close(running.server); });
  const host = await createRoom(running, 3);
  await post(running.base, "/api/join", {
    roomCode: host.roomCode, clientId: "p2", name: "二号", intent: "play"
  });
  const observers = [];
...
test("game9 SQLite restart restores spectator identity and room settings", async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "tablegame-game9-spectator-"));
  ...
  assert.equal(resumed.payload.view.players.every((player) => player.chips === null), true);
});

test("game9 enforces its spectator limit and expires with only a spectator online", async (context) => {
  const running = await launch({ spectatorLimit: 1, roomIdleMs: 100 });
  const streams = [];
  context.after(async () => { if (running.server.listening) await close(running.server, streams); });
  const host = await createRoom(running, 3);
  const observer = (await post(running.base, "/api/join", {
  ...
  const overflow = await post(running.base, "/api/join", {
    roomCode: host.roomCode, clientId: "watch-2", name: "额外旁观者", intent: "spectate"
  });
  assert.equal(overflow.response.status, 409);
  assert.equal(overflow.payload.code, "spectator_limit_reached");
  streams.push(await openEvents(running.base, observer));
  await new Promise((resolve) => setTimeout(resolve, 350));
  const expired = await post(running.base, "/api/join", {
    roomCode: host.roomCode,
...
test("game9 page exposes spectator controls and collapses the list on mobile", () => {
