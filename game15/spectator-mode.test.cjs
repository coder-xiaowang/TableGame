...
test("game15 supports voluntary spectators and safe lobby seat changes", async (context) => {
  ...
  const kicked = await post(running.base, "/api/kick", {
    roomCode: host.roomCode, hostId: host.clientId, resumeToken: host.resumeToken, playerId: player.clientId
  });
  assert.equal(kicked.response.status, 200);
});

test("game15 spectators never receive racks or a current player's private draft", async (context) => {
  const running = await launch();
  const streams = [];
  ...
  assert.equal((await post(running.base, "/api/actions", auth(current, {
    actionId: "begin-private-draft", action: { type: "beginEdit" }
  }))).response.status, 200);
  ...
  assert.equal(late.payload.view.turnEdited, true);
  assert.deepEqual(late.payload.view.hand, []);
  assert.deepEqual(late.payload.view.table, []);
  assert.equal(late.payload.view.canChangeSeats, false);
  assert.ok(late.payload.view.players.every((player) => !("hand" in player) && player.handCount === 14));
  const serialized = JSON.stringify(late.payload.view);
  assert.equal(serialized.includes("originalHandIds"), false);
  assert.equal(serialized.includes("handIds"), false);
  assert.equal(serialized.includes("undo"), false);
  const forged = await post(running.base, "/api/actions", auth(late.payload, {
    actionId: "forged", action: { type: "draw" }
  }));
...
test("game15 serializes concurrent competition for the last player seat", async (context) => {
  const running = await launch();
  context.after(async () => { if (running.server.listening) await close(running.server); });
  const host = await createRoom(running, 2);
  const observers = [];
...
test("game15 SQLite restart restores spectator identity, settings and empty rack", async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "tablegame-game15-spectator-"));
...
test("game15 page consumes shared spectator UI and hides its rack editor", () => {
