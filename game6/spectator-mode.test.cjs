...
async function launch(options={}) {
  ...
  const server = startAuthoritativeGameServer({
    ...
    protocolVersion:3,defaultPort:0,spectatorsEnabled:true,spectatorLimit:10,
    roomStore:store,...options
  });
...
const auth = (session, extras={}) => ({
  roomCode:session.roomCode,playerId:session.clientId,resumeToken:session.resumeToken,...extras
});
...
test("game6 supports voluntary spectators and lobby-only seat changes",async (context) => {
  ...
  assert.ok(observer.view.players.every((item) => item.hand.every((card) => card === null)));
  ...
  const kicked = await post(running.base,"/api/kick",{
    roomCode:host.roomCode,hostId:host.clientId,resumeToken:host.resumeToken,
    playerId:player.clientId
  });
  assert.equal(kicked.response.status,200);
});

test("game6 full and active rooms expose no hand or locked-card secret to spectators",async (context) => {
  const running = await launch();
  const streams = [];
  ...
  const hostView = (await post(running.base,"/api/join",{
    roomCode:host.roomCode,clientId:host.clientId,resumeToken:host.resumeToken
  })).payload.view;
  const chosen = hostView.players.find((player) => player.id === host.clientId).hand[0];
  assert.equal((await post(running.base,"/api/actions",auth(host,{
    actionId:"secret-select",action:{type:"selectCard",card:chosen}
  }))).response.status,200);
  ...
  assert.ok(late.payload.view.players.every((player) => player.hand.every((card) => card === null)));
  const selectedHost = late.payload.view.players.find((player) => player.id === host.clientId);
  assert.equal(selectedHost.hasSelected,true);
  assert.equal(selectedHost.selectedCard,null);
  assert.equal("playQueue" in late.payload.view,false);
  const forged = await post(running.base,"/api/actions",auth(late.payload,{
    actionId:"forged",action:{type:"selectCard",card:chosen}
  }));
  ...
  const closed = await post(running.base,"/api/room-settings",{
    roomCode:host.roomCode,hostId:host.clientId,resumeToken:host.resumeToken,allowSpectators:false
  });
...
test("game6 serializes concurrent competition for its last player seat",async (context) => {
  const running = await launch();
  context.after(async () => { if (running.server.listening) await close(running.server); });
  const host = await createRoom(running,2);
  const observers = [];
...
test("game6 SQLite restart restores spectator identity, settings and redacted view",async (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(),"tablegame-game6-spectator-"));
  ...
  assert.ok(resumed.payload.view.players.every((player) => player.hand.every((card) => card === null)));
});

test("game6 page consumes the shared spectator UI and hides its private hand panel",() => {
  ...
  assert.match(script,/setHidden\(E\.turnHandPanel,memberRole === "spectator"\)/);
