...
game6 已接入共享服务器权威演出层。秘密锁牌只公开完成状态；全员锁定后由服务器统一公开牌面，并依次表现入列、选列、收列罚分、下一回合和最终胜负。原有服务器阶段驱动的飞牌与收列动画继续负责物件移动，共享时间线负责方向轨迹、目标强调和中央播报，不会重复生成第二套飞牌。

演出队列支持快速追赶和高优先级结算保留，首次进入或重连不会重播历史。具体事件边界与实机验收项见 [PRESENTATION_MIGRATION.md](PRESENTATION_MIGRATION.md)。
...
移动端将 ACTION、倒计时和手牌合并为牌列顶部的吸附操作托盘。尚未选牌时，10张手牌以单行横向滚动方式展开；选牌锁定后以及公开、放牌、选列阶段，手牌自动收起，只保留紧凑的阶段提示和倒计时，避免在手牌与四个牌列之间反复纵向滚动。
