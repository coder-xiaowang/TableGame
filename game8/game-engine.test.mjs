...
test("game8 server starts deterministically and owns all player actions", () => {
  ...
  assert.deepEqual(state.presentationEvents.slice(-2).map((event) => event.kind), ["tokens-taken", "turn-start"]);
...
test("game8 waits for mandatory discards, then automatically ends when no evolution remains", () => {
  ...
  const publicReserve = state.presentationEvents.find((event) => event.kind === "card-reserved");
  assert.equal(publicReserve.cardId, cardId);
...
test("game8 views expose market reserves but hide deck reserves and deck order", () => {
  ...
  const secretCardId = secretState.game.players[secretSeat].reserve[0];
  const secretPublic = engine.buildSpectatorView(secretState).presentationEvents.find((event) => event.kind === "secret-reserve");
  const secretOwn = engine.buildView(secretState, secretActorId).presentationEvents.find((event) => event.kind === "secret-reserve-private");
  assert.ok(secretPublic);
  assert.equal(Object.hasOwn(secretPublic, "cardId"), false);
  assert.equal(JSON.stringify(secretPublic).includes(secretCardId), false);
  assert.equal(secretOwn.cardId, secretCardId);
  assert.ok(secretOwn.private);
  assert.ok(engine.buildView(secretState, secretOtherId).presentationEvents.every((event) => event.kind !== "secret-reserve-private"));
...
test("game8 restores legacy snapshots without presentation data", () => {
  const state = readyLobby();
  engine.applyAction(state, "host", { type: "start" }, { random: () => 0.27, now: 1000 });
  ...
  const seat = restored.game.turn;
  const actorId = restored.players[seat].id;
  engine.applyAction(restored, actorId, { type: "take", colors: ["red", "blue", "black"] }, { now: 2000 });
  assert.equal(restored.presentationEvents[0].sequence, 1);
  assert.equal(restored.presentationEvents[0].sceneId, "pokemon_scene_1");
});
